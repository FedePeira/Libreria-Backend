...
const resolvers = {
  Query: {
    ...
    allAuthors: () => {
      ...
      return authors.map(author => {
        const authorBooksCount = authorBooks[author.name] ? authorBooks[author.name].length : 0
        return {
            ...author,
  ...
  Mutation: {
    ...
    editAuthor: (root, args) => {
        ...
        if (!args.name || args.setBornTo === undefined) {
          return null
        }
        ...
        authors = authors.map(a => a.name === args.name ? authorUpdated : a)
